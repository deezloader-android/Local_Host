...
// Folders
const coverArtFolder = localpaths.home + path.sep + 'Deezloader Music' + path.sep + 'deezloader-imgs' + path.sep
...
io.sockets.on('connection', function (s) {
	...
	.then(body=>{
		...
		if (
			...
			s.emit("message", {title: `¡La versión ${lastVersion_MAJOR}.${lastVersion_MINOR}.${lastVersion_PATCH} está disponible!`, msg: body.changelog})
		}
	})
	.catch(error=>{
	...
	*/
	...
	async function downloadSpotifyPlaylist(data){
		...
		if (spotifySupport){
			...
			s.emit("message", {title: "Soporte de Spotify no está habilitado", msg: "Debe agregar authCredentials.js en los archivos de configuración para utilizar esta función.<br>¡Puede ver cómo hacer eso en <a href=\"https://notabug.org/RemixDevs/DeezloaderRemix/wiki/Spotify+Features\">esta guía</a>!"})
		}
	}
